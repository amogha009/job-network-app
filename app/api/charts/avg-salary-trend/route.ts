...
export async function GET() {
  try {
    ...
    result.forEach(row => {
        chartDataMap.set(row.month, row.average_salary ? parseFloat(row.average_salary) : null);
    });
    ...
    console.log("[API] Avg Salary Trend Data:", JSON.stringify(finalChartData));
    return NextResponse.json(finalChartData);

  } catch (error) {
    console.error('Failed to fetch average salary trend data:', error);
    return NextResponse.json(
        { error: 'Failed to fetch average salary trend data', details: error instanceof Error ? error.message : String(error) },
        { status: 500 }
    );
  }
} 